...
export class AppController {
  names = ['Devis', 'Ilya', 'Katya', 'Aleksei', 'John', 'Andrew', 'Max', 'Ann', 'Tanya', 'Kira', 'Sanofi'];
  surnames = ['Tolle', 'Petrow', 'Mask', 'Johnson', 'Messi', 'Lavrich', 'Pompaev', 'Zolotarev', 'Golotvin', 'Many'];
    ...
    for (const name of this.names) {
      for (const surname of this.surnames) {
        this.users.create({
          ...
          lastName: surname,
          password: '123456',
          email: `${name}${surname}@${this.domens[randomInt(this.domens.length)]}`
        }).then(user => {
          if (user) {
            user.hourlyRate = randomInt(3) ? randomInt(150) : null;
            user.english = randomInt(6);
